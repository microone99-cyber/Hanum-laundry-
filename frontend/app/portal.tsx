...
export default function Portal() {
  ...
  const categories = useMemo(
    () => [
      "Semua",
      ...Array.from(new Set(services.map((s) => s.kategori))),
    ],
    [services]
  );
  ...
  const filteredServices =
    cat === "Semua"
      ? services
      : services.filter((s) => s.kategori === cat);
  ...
  const claim = async () => {
    ...
    try {
      const r = await api.post("/orders/claim", {
        kode: kode.trim(),
      });
  ...
  const pesan = async () => {
    ...
    try {
      ...
      const catatanFull = [antarInfo + paketNama, catatan]
        .filter(Boolean)
        .join(" | ");
  ...
  const totalPcs = cart
    .filter((i) => i.satuan === "pcs")
    .reduce((a, i) => a + i.harga * i.qty, 0);
  ...
  return (
    ...
    >
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              subtitle="Klaim nota dari struk atau buat pesanan baru."
            />
          }
          renderItem={({ item }) => {
            const bisaBatal =
            ...
            return (
              <Card testID={`portal-order-${item.id}`}>
                ...
                >
                  Lacak: {item.kode_tracking} ·{" "}
                  {tglID(item.created_at)}
                </AppText>
      ...
      <Sheet
        visible={claimSheet}
        onClose={() => setClaimSheet(false)}
        title="Klaim Pesanan"
        testID="claim-sheet"
      >
        <View style={{ gap: SP.md }}>
          <AppText style={{ color: C.muted }}>
            Masukkan kode nota (tercetak di struk laundry Anda).
          </AppText>
      ...
      >
        ...
        >
          ...
          >
            ...
            {"\n"}
            Cucian kiloan dihitung petugas saat ditimbang; item
            satuan (pcs) langsung pasti harganya.
          </AppText>
          ...
          {cart.length > 0 && (
            <View style={{ marginTop: SP.lg }}>
              ...
              >
                ...
                >
                  *Harga kiloan dihitung petugas saat ditimbang
                </AppText>
              </Card>
